...
\newchapter{QUThesis User Manual}
...
The quthesis package is designed to be used with the \code{book} documentclass, though it should also be compatible with \code{memoir}. The package is initialized in the typical manner,
...
\item Customized \code{listings} environment to better match the prose formatting
\item Better default settings for \code{hyperref}, including link colors, PDF metadata and references
...
Format a quote, like the one at the beginning of the manual. The command has two optional arguments. The \code{author} formats a right-justified attribution after the quote. The \code{alignment} argument specifies the horizontal alignment of the quote on the page. It may be one of \code{flushleft|center|flushright}.
...
\item \code|\begin{biography}[path/to/portrait.png] ... \end{biography}| \\
...
\begin{itemize}
\item \code|\alphachapter[Alpha Name]{Chapter Name}| \\
A custom unnumbered chapter that appears like regular chapters, except with an alpha name. This is useful for introductory chapters such as \code|\alphachapter[Introduction]{The State of the Union}|.
\item \code|\chap, \sec, \subsec, \fig, \eqn| \\
These commands wrap \code|\ref| to provide two utilities: (i) enforce namespacing in the use of labels, and (ii) enforce consistency of referring to labels. A figure with the label \code|\label{fig:results}| for example, can be referred to by \code|\fig{results}| which will evaluate to \code{Figure 2}. In the five variants, the namespace in the label is always the command name followed by a colon.
\item \code|\code{inline code}| \\
An alias of the \code{lstinline} command.
\item \code|\ie, \eg, \cf, \etc, \wrt, \dof, \etal| \\
Italicized abbreviations which respect the spacing and deduplication (for sentence ending abbreviations) of the final period: \ie, \eg, \cf, \etc, \wrt, \dof, \etal.
\end{itemize}
